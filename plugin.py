...
class BasePlugin:
    ...
    def onHeartbeat(self):
        ...
        if self.MeteoRequest <= now:
            ...
            if jsonData:
                # on recup les datas
                self.OutTemp = int(jsonData['current']['temp'])
                self.FeelTemp = int(jsonData['current']['feels_like'])
                self.TodayMinTemp = int(jsonData['daily']['temp']['min'])
                self.TodayMaxTemp = int(jsonData['daily']['temp']['max'])
                Domoticz.Debug("Current outdoor temp = {}".format(self.OutTemp))
                Domoticz.Debug("Current feel temp = {}".format(self.FeelTemp))
                Domoticz.Debug("Current Min Temp = {}".format(TodayMinTemp))
                Domoticz.Debug("Current Max temp = {}".format(TodayMaxTemp))
