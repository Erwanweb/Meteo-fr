...
class BasePlugin:
    ...
    def onHeartbeat(self):
        ...
        if lat = 0 :
            jsonData = DomoticzAPI("type=command&param=getsettings")
            if jsonData:
                lat = round(float(jsonData['location']['Latitude']), 5)
                lon = round(float(jsonData['location']['Longitude']), 5)
        ...
        if self.MeteoRequest <= now:
            ...
            if jsonData:
                ...
                self.WindWg = round(float((jsonData['current']['wind_gust'])*10), 1)
