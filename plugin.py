...
def WeatherMapAPI(APICall):
    ...
    try:
        ...
        if response.status == 200:
            ...
            self.OutTemp = str(resultJson'current']['temp'])
            self.FeelTemp = str(resultJson['current']['feels_like'])
            self.TodayMinTemp = str(resultJson['daily']['temp']['min'])
            self.TodayMaxTemp = str(resultJson['current']['temp']['max'])
