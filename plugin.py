    ...
    def onHeartbeat(self):
        ...
        if self.MeteoRequest <= now:
            ...
            if jsonData:
                ...
                self.WindDir = "N"
                self.WindWS = round(float(jsonData['current']['wind_speed']), 1)
                self.WindWg = round(float(jsonData['current']['wind_gust']), 1)
                ...
                # default is 90,N,16,25,20º,21º
                Devices[4].Update(nValue=0, sValue="{};{};{};{};{};{}".format(str(self.WindDeg), self.self.WindDir, str(self.WindWS), str(self.WindWg), str(self.OutTemp), str(self.FeelTemp)))
