...
class BasePlugin:
    ...
    def onHeartbeat(self):
        ...
        if self.MeteoRequest <= now:
            ...
            CurrentMeteoAPI = WeatherMapAPI("")
            if CurrentMeteoAPI:
                #for CurrentData in CurrentMeteoAPI["current"]:
                if "Temp" in CurrentMeteoAPI["current"]:
                    Domoticz.Debug("Current outdoor temp= {}".format(["Temp"]))
                    #self.OutTemp =  round(CurrentData['temp'], 1)
