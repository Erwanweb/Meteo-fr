...
def WeatherMapAPI(APICall):
    ...
    self.OutTemp request.json()['current']['temp']
    self.FeelTemp = request.json()['current']['feels_like']
    self.TodayMinTemp = request.json()['daily']['temp']['min']
    self.TodayMaxTemp = request.json()['current']['temp']['max']
    """
    try:
