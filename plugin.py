...
"""
<plugin key="METEO-FR" name="Meteo plugin from Ronelabs" author="Ronelabs" version="0.0.2" externallink="https://ronelabs.com">
      <description>
...
class BasePlugin:
    ...
    def onHeartbeat(self):
        ...
        if self.MeteoRequest <= now:
            self.MeteoRequest = datetime.now() + timedelta(minutes=30)  # make make an API Call every 30 minutes, to have max 50 per days
            ...
            #WeatherMapAPI("")
            jsonData = WeatherMapAPI("&lat={}&lon={}&appid={}".format(str(self.lat), str(self.lon), str(self.APIK)))
            if jsonData:
                # on recup les datas
...
def WeatherMapAPI(APICall):
    ...
    jsonData = None
    url = f"https://api.openweathermap.org/data/3.0/onecall?units=metric&lang=fr&exclude=minutely,hourly{parse.quote(APICall, safe='&=')}"
