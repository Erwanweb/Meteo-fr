...
class BasePlugin:
    ...
    def onStart(self):
        ...
        devicecreated = []
        if 1 not in Devices:
            Domoticz.Device(Name="Actual Temp", Unit=1, TypeName="Temp+Hum").Create()
        ...
        if 3 not in Devices:
            Domoticz.Device(Name="TempHumBaro", Unit=3, TypeName="Temp+Hum+Baro").Create()
            devicecreated.append(deviceparam(3, 0, "20;50;0;0"))  # default is 20º,21º
        if 4 not in Devices:
            Domoticz.Device(Name="Wind", Unit=3, TypeName="Wind").Create()
            devicecreated.append(deviceparam(4, 0, "90;N;16;25;20;21"))  # default is 90,N,16,25,20º,21º
        if 5 not in Devices:
            Domoticz.Device(Name="TempHumBaroDay0", Unit=5, TypeName="Temp+Hum+Baro").Create()
            devicecreated.append(deviceparam(5, 0, "20;50;0;0"))  # default is 20º,21º
        if 6 not in Devices:
            Domoticz.Device(Name="Today Min. Temp", Unit=6, TypeName="Temperature").Create()
            devicecreated.append(deviceparam(6, 0, "20"))  # default is 20 degrees
        if 7 not in Devices:
            Domoticz.Device(Name="Today Max. Temp", Unit=7, TypeName="Temperature").Create()
    ...
    def onHeartbeat(self):
        ...
        if self.MeteoRequest <= now:
            ...
            if jsonData:
                ...
                # DJUs
                # self.dju0 = ((self.TodayMinTemp + self.TodayMaxTemp) / 2)
                #Domoticz.Debug("DJU Previsionnels = {}".format(self.dju0))

                # Updating devices values
                Domoticz.Debug("Updating Devices from meteo datas")
                #Devices[1].Update(nValue= 0, sValue=str(self.OutTemp))
                Devices[1].Update(nValue= 0, sValue="{};{}".format(str(self.OutTemp), str(self.OutHum)))
